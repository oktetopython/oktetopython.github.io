...
function drawFlute() {
  if ($('.calc-error').length) {
   ...
   var canvas = $('canvas#flute-canvas')[0];
    ...
    ctx.fillStyle = '#fff';
    ctx.fillRect(1, cH_2 - 0.5 * scale * borD, cW - 2, scale * borD);


	 ctx.fillStyle = 'black';
    ctx.beginPath();
    ctx.arc(cW - scale * embX, cH_2, 0.5 * scale * actEmbD, 0, 2 * Math.PI);
    ctx.fill();

    for (var j = 1; j <= holeCount; j++) {
        var i = holeCount + 1 - j;
        var holeX = cW - scale * $('input#result' + j + '').getValue();
        var holeRadius = 0.5 * scale * fhDs[i];
		ctx.beginPath();
        ctx.arc(holeX, cH_2, holeRadius, 0, 2 * Math.PI);
        ctx.fill();
    }
